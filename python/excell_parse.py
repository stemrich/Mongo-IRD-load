...
from utils import split_str_array, get_cell, geocode  # ,na_check
...
def parse(sheet, data):
    ...
    labels = sheet.row(0)
    lkey = {str(labels[i]).replace("text:u", "").replace("'", "").lower():
    ...
    for row_index in range(1, nrows):
        ...
        a = get_cell(sheet, 'Subject', row_index, lkey,
                     str_split="([:;, ']|\. |\.$)")
        if a:
            # input control
            b = []
        ...
        if a:
            if a == 'No information':
                ...
                for x in xrange(1, len(activity_name_array)):
                    try:
                        activity_array.append({'activity_name':
                                              activity_name_array[x],
                                              'activity_description':
                                               activity_description_array[x]})
                    except IndexError:
        ...
        if a:
            if a == 'No information':
                ...
                IRD_activities_reg = re.split('[0-9]{1,2}\. ',
                                              get_cell(sheet, 'IRDALocation',
                                                       row_index, lkey))
        ...
        if not a:
            ...
            try:
                ...
                entry_date = datetime.strptime(entry_value_array[1],
                                               "%d.%m.%Y").date()
                data[-1]["entry"] = {'author': entry_value_array[0],
                                     'date': str(entry_date.year) +
                                     str(entry_date.month).zfill(2) +
                                     str(entry_date.day).zfill(2)}
            except Exception:
